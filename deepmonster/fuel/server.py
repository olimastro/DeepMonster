...
def server_stream(dataset=None, stream=None, hwm=10, wrapping_streams=[],
    ...
    """This function is the main utility to use fuel.ServerDataStream in conjunction
       ...
       ***This function has 2 different mutually exclusive modes:
       1) dataset is not None := it will start a stream and do all the background work
    ...
    if stream is None:
        assert len(dataset) == 3, "dataset needs to be a tuple of (dataset name or object, split, batch_size)"
        dataset, split, batch_size = dataset
